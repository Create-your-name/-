...
import { getEqpQcList } from '@/pages/EqpManage/EqpQcShow/EqpQcShow';
...
const EqpListShow = () => {
  ...
  useEffect(() => {
    setInterval(async () => {
      let resp = await getEqpQcList();
      eqpList = resp.data.map((e: { RESULT: string; }) => {
        let color = getEqpStatuscolor(e.RESULT);
        return { ...e, color: color };
      });
      eqpList.map((e) => {
        if (document.getElementById(e.EQPID) != null) {
          if (e.color != document.getElementById(e.EQPID)?.style.backgroundColor) {
            let do1 = document.getElementById(e.EQPID);
