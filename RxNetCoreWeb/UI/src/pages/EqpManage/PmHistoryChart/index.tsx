...
import { getPMHisChartCheckList, getPMChartPoint } from './pmBmHistory';
...
const PmHistoryChart = () => {
  ...
  const onSearch = (values: any) => {
    ...
    (async () => {
      let resp = await getPMChartPoint(values);
  ...
  const handleEqpTypeChange = (value: string) => {
    searchForm.setFieldsValue({ [(() => "EQPID")()]: null, [(() => "PMFORMID")()]: null, [(() => "CHKINDEX")()]: null });
  ...
  const handleUpdateChartChecklist = (value: string) => {
    (async () => {
      ...
      let resp = await getPMHisChartCheckList({ SYSID: searchForm.getFieldValue("PMFORMID") });
      setCurrentChkIndex(
        resp.data.map((e: any) => {
          return {
            value: e.CHKINDEX,
      ...
      <Card style={{ marginBottom: 20 }}>
        <Form style={{ marginTop: 20 }} form={searchForm} onFinish={onSearch}>
          <Row gutter={20}>
            ...
            <Col span={8}>
              <Form.Item label="PM表单" name="PMFORMID" rules={[{ required: true, message: '请选择PM表单!' }]}>
