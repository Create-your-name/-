...
import { makeLineAnnotations, pointAnnotation } from '../../ChartComponents/ChartUtils';
import { each, findIndex } from '@antv/util';
import { Datum } from '@ant-design/charts';

const DemoLine = ({ collection, size, controlLine }:
    {
        collection: any[] | undefined,
        size: { width: number, height: number }
        controlLine: { max: number, min: number }
    }) => {
    ...
    annotations.push(makeLineAnnotations(controlLine.max, "最大值:" + controlLine.max, "red"));
    annotations.push(makeLineAnnotations(controlLine.min, "最小值:" + controlLine.min, "red"));
    ...
    const config = {
        ...
        yAxis: {
            minLimit: parseInt((controlLine.max * 1.2 + 1).toString()),
            maxLimit: parseInt((controlLine.min * 0.8 - 2).toString()),
        },
        xAxis: {
            tickCount: 8,
        },
        ...
        tooltip: {
            // showMarkers: false,
            customContent: (title: any, item: any): any => {
                if (item[0]) {
                    return `<div><br/><p>日期: ${title}</p></div><div><p>设备: ${item[0].data.EQPID}</p></div><div><p>数值: ${item[0].data.value}</p></div>`;
                }
            }
        },
        state: {
